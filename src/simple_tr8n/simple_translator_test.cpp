...
TEST_F(SimpleTranslatorCharTest, ShouldTranslateWithNoArgs) {
  ...
  EXPECT_THAT(esTranslator->translate(test_msgs::kNoArgs),
      Eq("Un mensaje simple sin argumentos"));
}
...
TEST_F(SimpleTranslatorCharTest, ShouldTranslateWithArgs) {
  EXPECT_THAT(
      ...
      Eq("hello, Bob!"));
  EXPECT_THAT(
      esTranslator->translate(test_msgs::kHelloName, {{"personName", "Bob"}}),
      Eq("hola, Bob!"));
  ...
  EXPECT_THAT(
      ...
      Eq("progress: 75%"));
  EXPECT_THAT(
      esTranslator->translate(test_msgs::kProgressPct, {{"pct", "75"}}),
      Eq("progreso: 75%"));
...
TEST_F(SimpleTranslatorCharTest, ShouldTranslatePlural) {
  ...
  EXPECT_THAT(
      esTranslator->translatePlural(test_msgs::kCoupleFishCount, 1, {
        {"person1Name", "Alice"},
        {"person2Name", "Bob"},
        {"fishCount", "1"},
      }),
  ...
  EXPECT_THAT(
      esTranslator->translatePlural(test_msgs::kCoupleFishCount, 2, {
        {"person1Name", "Alice"},
        {"person2Name", "Bob"},
        {"fishCount", "2"},
      }),
  ...
  EXPECT_THAT(
      esTranslator->translatePlural(test_msgs::kCoupleFishCount, 3, {
        {"person1Name", "Alice"},
        {"person2Name", "Bob"},
        {"fishCount", "3"},
      }),
  ...
  EXPECT_THAT(
      esTranslator->translatePlural(test_msgs::kCoupleFishCount, 102, {
        {"person1Name", "Alice"},
        {"person2Name", "Bob"},
        {"fishCount", "102"},
      }),
...
TEST_F(SimpleTranslatorCharTest, ShouldHandleErrorsPlural) {
  ...
  try {
    ...
    EXPECT_THAT(e.what(),
        StrEq("simple_tr8n::MissingArgException: (msgType) test.couple_fish_count: (argKey) person2Name"));
  }

  // No matching plural case (<= count):
  try {
    esTranslator->translatePlural(test_msgs::kCoupleFishCount, 0, {
      {"person1Name", "Alice"},
      {"person2Name", "Bob"},
      {"fishCount", "0"},
    });
...
protected:
  void SetUp() override {
    ...
    enConfig->add(test_wmsgs::kNoArgs, L"A simple message with no arguments")
        ...
        .add(test_wmsgs::kCoupleFishCount, {
...
TEST_F(SimpleTranslatorWCharTest, ShouldWorkWithWideChars) {
  // Success case:
  EXPECT_THAT(
      enTranslator->translatePlural(test_wmsgs::kCoupleFishCount, 6, {
